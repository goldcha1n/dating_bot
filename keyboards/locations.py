...
def regions_kb(regions: list[str]) -> InlineKeyboardMarkup:
    ...
    builder.button(text="Інша (введу сам)", callback_data="loc:r:other")
    builder.adjust(2)
...
def districts_kb(districts: list[str]) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(text="Міста області (без району)", callback_data="loc:d:region_cities")
...
def hromadas_kb(hromadas: list[str]) -> InlineKeyboardMarkup:
    ...
    builder.button(text="Інша (введу сам)", callback_data="loc:h:other")
    builder.adjust(2)
    return builder.as_markup()
...
def settlements_kb(settlements: list[str]) -> InlineKeyboardMarkup:
    ...
    builder.button(text="Інший населений пункт", callback_data="loc:s:other")
    builder.adjust(2)
    return builder.as_markup()
